...
sapply(v, evenodd1) # 벡터의 원소만큼 함수 호출 / 리턴 값이 가급적 단순한 객체
lapply(v, evenodd1) # list 객체  
...
test1 <-function(p){
  cat("난 수행함\n")
  testError1(-1)  # 여기서 함수가 종료됨 testError와 test1 둘다 중단
  cat("나 수행할 까요? \n")
}
test1()
...
test2 <- function(p){
  cat("난 수행함\n")
  try(testError1(-1)) #try함수는 에러가 나도 함수가 계속 진행하게 함
  cat("나 수행할 까요? \n")
}
test2()
...
f.case1(100)
f.case1(NA)
f.case1(1:6)           # 첫번째 원소만 보고 판단하므로 에러생김
f.case1(c(10,20,30))
f.case1(c(NA, 20))
f.case1(c(10, NA, 20))

f.case2 <- function(x) {
  if(any(is.na(x)))    # any() 전부 확인 
    return("NA가 있슈")
  else
    return("NA가 없슈")
}
