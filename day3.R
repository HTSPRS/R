...
a[[1]][1]
a$a[1]
a[1]+1 # 리스트 구조라 연산은 에러
a[[1]]+1
a[[1]][2] <- 100 # 리스트 안 값 변경
new_a <- unlist(a[1]) # 기존에 name인 a를 반영해 name이 자동으로 생성
a[1]; new_a
names(a) <- NULL
names(new_a) <- NULL
...
data <- "가나다"
print(data)
print(data, quote=FALSE) #  quote 사용으로 "" 인용부호 생략됨
v1 <- c("사과", "바나나", "포도")
print(v1)
print(v1, print.gap=10) # 사이 띄우기 
cat(100)
cat(100,200)
cat(100,200,"\n")
cat("aaa", "bbb", "ccc", "ddd", "\n")
